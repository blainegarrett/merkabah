        ...
        if encoded_post_data or no_cache:
          if encoded_post_data:
              try:
                  ...
                  raise Exception('POST VERSION: Unable to connect to the oAuth Service. Code: %s  - Url: %s : Content - %s' % (e.code, e.url, e.msg)) # Important for debugging
          else:
              try:              
                  url_data = opener.open(url).read()
              except urllib2.HTTPError, e:
                  raise Exception('GET VERSION: Unable to connect to the oAuth Service. Code: %s  - Url: %s : Content - %s' % (e.code, e.url, e.msg)) # Important for debugging                  
          opener.close()
        else:
